...
const Service = () => {
  ...
  const options = categoryArray?.map((item) => {
    const newObject = { value: item?.categoryTitle };
    return newObject;
  });
  const options2 = categoryArray?.map((item) => {
    const newObject = { value: item?.categoryTitle };
    return newObject;
  });
  ...
  const serviceSaveClick = () => {
    if (
      ...
      if (categoryObject) {
        ...
        toast.error(`"${category}" category doesn't exist !`);
      }
    } else {
      toast.custom(
        <div
  ...
  const updateServiceClick = (
    ...
    //  console.log(_id, url, title, categoryId, price, des)
    setServiceForm(() => {
      return {
        url,
        title,
    ...
    setCategory(categoryObject?.categoryTitle);
    setServiceId(_id);
  };
...
//console.log(categorySelect)
    ...
    servicesDisplay = service?.map((item) => {
      if (categorySelect === "") {
        ...
        const categoryObject = categoryArray?.find(
          (item) =>
            item?.categoryTitle?.toLowerCase().trim() ===
            categorySelect.toLowerCase().trim()
        );

        if (categoryObject?._id === item?.categoryId) {
          return (
            <ServiceCard
              key={item._id}
              {...item}
  ...
  return (
    <>
      ...
      <div
        ...
        <div
          style={{
            ...
            marginTop: "30px",
            marginBottom: "30px",
            flexDirection: screen < 805 ? "column" : "row",
          }}
        >
          <section style={{ width: screen < 805 ? "100%" : "40%" }}>
            ...
            >
              ...
              {serviceId !== "" && (
                <button
                  className="save-service"
                  onClick={updateServiceButtonClick}
                  style={{
                    marginTop: "10px",
                    marginBottom: "20px",
                    cursor: "pointer",
        ...
        {service?.length >= 1 && (
          ...
          >
            ...
            >
              You have added {service?.length} services
            </p>
          </div>
        )}
      ...
      <Backdrop sx={{ color: "gold" }} open={serviceLoading}>
        <CircularProgress color="inherit" />
      </Backdrop>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            onClick={deleteServiceButtonClick}
            variant="contained"
            color="error"
          >
